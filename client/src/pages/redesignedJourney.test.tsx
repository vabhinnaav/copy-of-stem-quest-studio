...
import { loadOrCreateLocalLearner, signOutLocalLearner } from "@/lib/localProgress";
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    stem: {
      generateQuestion: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) },
      submitAnswer: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) },
      followUp: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) },
    },
    mentor: { chat: { useMutation: () => ({ mutate: vi.fn(), isPending: false }) } },
  },
}));
...
describe("redesigned learner journey UI", () => {
  it("moves from landing through setup into the real workspace and exposes the working profile, mentor, subject, and follow-up controls", async () => {
    ...
    const first = render(<><Home /><DeviceProfileManager /><DeviceAIMentor /></>);

    expect(screen.getByRole("heading", { name: /practice with curiosity/i })).toBeTruthy();
    expect(screen.getAllByRole("button", { name: /start learning/i })).toHaveLength(2);
    expect(screen.getByRole("button", { name: /configure ai provider/i })).toBeTruthy();
    await user.click(screen.getAllByRole("button", { name: /start learning/i })[0]);
    expect(scrollIntoView).toHaveBeenCalled();
    await user.click(screen.getByRole("button", { name: /configure ai provider/i }));
    expect(screen.getByText(/bring your own ai provider/i)).toBeTruthy();
    await user.click(screen.getByRole("button", { name: /close provider settings/i }));

    await user.type(screen.getByLabelText(/learner name/i), "Ada");
    await user.click(screen.getByRole("button", { name: /search & enter workspace/i }));
    ...
    for (const subject of ["Science", "Technology", "Engineering", "Mathematics"]) expect(screen.getByRole("button", { name: new RegExp(subject, "i") })).toBeTruthy();
    ...
    expect(screen.getByRole("dialog", { name: /manage learner profile/i })).toBeTruthy();
    ...
    expect(screen.getByRole("heading", { name: /set your challenge/i })).toBeTruthy();
    expect(screen.getByText(/need more help/i)).toBeTruthy();

    first.unmount();
    window.history.pushState({}, "", "/?followUpPreview=1");
    render(<><Home /><DeviceProfileManager /><DeviceAIMentor /></>);
    expect(screen.getByRole("button", { name: /next question/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /ask about this question/i })).toBeTruthy();
  ...
  it("opens the actual profile and provider overlays from their registered global events", () => {
    ...
    render(<><DeviceProfileManager /><DeviceAIMentor /></>);

    fireEvent(window, new Event(JOURNEY_EVENTS.openProfile));
    expect(screen.getByRole("dialog", { name: /manage learner profile/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /switch learner/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /delete this learner profile/i })).toBeTruthy();

    fireEvent(window, new Event(JOURNEY_EVENTS.openMentorSettings));
    expect(screen.getByText(/bring your own ai provider/i)).toBeTruthy();
    expect(screen.getByRole("button", { name: /save session connection/i })).toBeTruthy();
  });
});
